<?php

return [
    'Yelyzaveta'=>'Єлизавета',
    'Lazarieva'=>'Лазарєва',
    'About'=> 'Про мене',
    'Resume'=>'Резюме',
    'My resume'=>'Моє резюме',
    'Portfolio'=>'Портфоліо',
    'My portfolio'=>'Моє портфоліо',
    'Contact'=>'Контакти',
    'Contact me'=>'Мої контакти',
    'Name'=>'Ім\'я',
    'Website'=>'Сайт',
    'Phone'=>'Телефон',
    'City'=>'Місто',
    'Kharkiv, Ukraine'=>'Харків, Україна',
    'Łódź, Poland' => 'Лодзь, Польща',
    'Age'=>'Вік',
    'Education'=>'Освіта',
    'Birthday'=>'Дата народження',
    'Skills'=>'Мої навички',
    'Summary'=>'Загальне',
    'Professional Experience'=>'Досвід роботи',
    'Random facts'=>'Рандомні факти',
    'V.N.Karazin Kharkiv National University'=>'ХНУ ім. В.Н.Каразіна',
    'Language'=>'Change language',
    'Home'=>'На головну',
    'June 2022 - June 2023'=>'Червень 2022 - Червень 2023',
    'VESCOM, Łódź, Poland'=>'VESCOM, Лодзь,Польща',
    'Work with database (SQL)'=>'Робота з базами даних (SQL)',
    'Work with YII2, Angular and .net frameworks'=>' Робота з фрейморками YII2, Angular та .net',
    'Make layouts responsive'=>'Створення адаптивних макетів',
    'Update legacy project to Bootstrap5'=>'Оновлення існуючого проєкту до Bootstrap 5',
    'V.N.Karazin'=>'Харківський національний університет ім. В.Н.Каразіна',
    'Faculty of Computer Science'=>'Факультет комп\'ютерних наук' ,
    'Speciality: Cyber-security'=>'спеціальність: Кібербезпека',
    'University of Łódź'=>'Лодзінський університет, Польща',
    'I studied at the University of Łódź as a mobility student for one semester. Taken courses:'=>'Я навчалася в Лодзінському університеті за обміном протягом одного семестру. Вивчала наступні курси:',
    'I am addicted to plants'=>'Я обожнюю кімнатні рослини',
    'I play guitar and piano'=>'Я граю на гітарі та піаніно',
    'I studied 6 languages (although some of them remained at the elementary level)'=>'Я вивчала 6 мов (хоча деякі з них так і залишилися на початковому рівні)',
    'I drink a looooot of tea'=>'Я п\'ю багаааато чаю',
    'I dream of having a cat'=>'Мрію завести кота',
    'In 2021 I won an award from \'She is Science\' for the best essay about female scientist'=>'У 2021 році я перемогла у конкурсі есеїв від "Наука - це вона"',
    'I enjoy cooking, especially soups'=>'Я люблю готувати, особливо супи',
    'Social Profiles'=>'Соціальні мережі',
    'My skills'=>'Навички',
    'I graduated from V.N.Karazin Kharkiv National University. I have one year of commercial experience in Frontend development.'=>'Нещодавно я закінчила Харківський національний університет імені В. Н. Каразіна. Маю рік комерційного досвіду у Front-End розробці.',
    'was my first attempt to create a web page.'=>' це моя перша спроба створити веб-сторінку.',
    'Building Materials'=>'Будматеріали',
    'It is a one-page website with adaptive layout. It was written without any frameworks - using bare HTML and CSS. Some JavaScript fragments were used to create responsive navigation and a "scroll to top" button.'=>'Це односторінковий сайт з адаптивним дизайном. Був написаний без використання фреймворків - чистий HTML та CSS. Використовувались фрагменти JavaScript для створення адаптивної навігації та кнопки "scroll to top".',
    'It was hosted on GitHub Pages.'=>'Сайт був розміщений на GitHub Pages.',
    'Link to Repo'=>'Посилання на репозиторій',
    'Building materials - online shop'=>'Онлайн-магазин "Будматеріали"',
    'My first page ever'=>'Моя найперша сторінка',
    'First Resume'=>'Перше резюме',
    'Made with bootstrap 5'=>'Зроблено з використанням bootstrap 5',
    'The aim of this project was to show my ability to work with bootstrap 5: create responsive tables, long descriptions, images and so on.'=>'Метою цього проєкту було показати вміння працювати з bootstrap 5: створювати адаптивні таблиці, довгі описи, використовувати картинки тощо.',
    'It is a one-page website with adaptive layout. It was written using HTML (bootstrap 5), CSS and some JavaScript for animations.'=>'Це односторінковий веб-сайт з адаптивною версткою. Його було написано з використанням HTML (bootstrap 5), CSS і трохи JavaScript для анімації.',
    'is a project for practicing the use of Angular.'=>'Це проєкт для практикування використання Angular',
    'It is a website that converts one currency into another. It was written using Angular.'=>'Це веб-сайт, який конвертує одну валюту в іншу. Він був написаний за допомогою Angular.',
    'For this website, I used an open API to get the current exchange rate for EUR, USD, UAH and PLN.'=>'Для цього веб-сайту я використовувала відкритий API, щоб отримати поточний курс обміну євро, доларів, гривень і злотих.',
    'was a kind of test project to show my bootstrap 5 skills.'=>'був своєрідним тестовим проектом, щоб показати мої навички використання Bootstrap 5.',
    'Made with Yii2'=>'Зроблено з використанням Yii2',
    'Made with Angular'=>'Зроблено з використанням Angular',
    'Made with JavaScript'=>'Зроблено з використанням JavaScript',
    'Initially, the website was supposed to be only for desktops. However in the end I decided to make it mobile friendly as well :)'=>'Спочатку веб-сайт мав бути лише для комп’ютерів. Однак зрештою я вирішила зробити його також mobile-friendly :)',
    'It is a website with three games. It was written using HTML, CSS and a looooot of JavaScript.'=>'Це веб-сайт із трьома іграми. Використовувалися HTML, CSS і багаааато JavaScript.',
    'is a project for practicing the use of JavaScript.'=>'це проєкт для практики використання JavaScript.',
    'Your name'=>'Ваше ім\'я',
    'Subject'=>'Тема',
    'Field must be filled'=>'Поле має бути заповнене',
    'I agree to the '=>' Я погоджуюсь із ',
    'Privacy Policy'=>'Політикою конфіденційності',
    'You must agree to the Privacy Policy.'=>'Ви повинні погодитися з Політикою конфіденційності.',
    'Message'=>'Повідомлення',
    'Loading'=>'Зававантаження',
    'Your message has been sent!'=>'Ваше повідомлення було надіслано!',
    'Send Message'=>'Надіслати повідомлення',
    'is written using Yii2, bootstrap5 and SASS.'=>'Написано з використанням Yii2, bootstrap5 та SASS.',
    'Source code could be found on GitHub.'=>'Вихідний код можна побачити на GitHub.',
    'This site demonstrates the skills I possess. To a greater extent, the emphasis is on Yii2 and Bootstrap5. Also presented on the site:'=>'Даний сайт демонструє навички, якими я володію. Більшою мірою акцент зроблено на Yii2 та Bootstrap5. Також на сайті представлені:',
    'Contact form'=>'форма зв\'язку;',
    'Image Gallery (Lightbox)'=>'галерея зображень (Lightbox);',
    'Moreover it is possible to change the language (by default, the language is selected according to the settings of your browser)'=>'До того ж є можливість зміни мови (за замовченням мова обирається відповідно до налаштувань вашого браузера).',
    'Your message has been sent! I will respond to you as soon as possible.'=>'Ваше повідомлення надіслано! Я відповім Вам якомога швидше.',
    'Something went wrong! Try again.'=>'Щось пішло не так! Спробуйте знову.',
    'Try again'=>'Спробувати знову',
    'Success'=>'Успіх',
    'Error'=>'Помилка',
    'Send one more'=>'Надіслати ще одне',
    'I am a front-end developer from Ukraine. I am not afraid of challenges and am ready to take on interesting tasks.'=>'Я фронт-енд розробник з України. Я не боюся викликів та готова братися за цікаві завдання.'
    ];
